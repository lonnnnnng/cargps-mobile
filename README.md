...
- 轨迹点按最多 16 点或 1 秒批量事务落库；普通持久化失败或磁盘持续不可写时，定位前台服务停止新增输入并显示“等待存储恢复”；磁盘不可写时内存只保留 16 个未确认点，第 17 个点同步拒绝。尾批成功落库并发布新检查点后自动恢复定位。存储层的永久只读写失败已在真实 Room/SQLite 连接上验证事务原子性，但物理 `ENOSPC` 和通知/GPS 恢复仍未完成设备验收。
