...
- 当前开发线：`v0.2.0` 之后的 M1-M7；M1-M7 已提交并完成 Pixel_9 核心验证，API 27/29/31/33 聚焦回归也已通过。
...
- M5 策略实现、11 个 JVM 场景和 Pixel_9 完整权限矩阵已通过；API 29 仅粗略定位、API 31 完整位置权限/设置返回/活动行程撤权、API 33 通知首次/永久拒绝均已实机化验证。剩余为 API 27/29/33 的完整位置拒绝、系统定位和设置返回矩阵。
