...
- 每次轨迹批次成功落库后发布并保留最近的已确认检查点，记录行程开始时间、确认点数、最后 sequence 和时间；背压期间保留活动行程、常驻通知和“结束行程”入口。任务被移除时由 Service 协程等待尾批次确认，但系统仍可能在等待完成前回收进程。存储失败或队列拒绝会切断定位连续段并清空速度平滑器，避免恢复首点跨故障窗口补算。
...
- M2 已在 API 27/API 29/API 31 完成开始、Home、单定位线程、结束和资源清理短路径；API 31 还通过了锁屏保持前台记录。Pixel_9 / API 35、Android 10 / API 29、Android 8.1 / API 27 分别完成 41 个样本、1831/1816/1816 秒回归；三轮均覆盖 Home、系统设置、锁屏和解锁返回，结束后资源完整清理。最新背压分支已在 Pixel_9/API 35 与 API 27 通过完整 `gps-core` 14/14、手机版 9/9 和 Runtime/Room 专项 1/1 回归；可恢复写失败已验证通知、定位停止和自动恢复编排，物理低存储与真实 GPS 注册仍是发布前门禁。
- M3 已在 Pixel_9、API 27 和 API 29 通过普通进程 `SIGKILL` 恢复验证；API 27 整机重启后活动行程数据保留但不会自动继续，用户打开应用后显示“已恢复”并重新建立服务。恢复上限是最后确认检查点：正常批次延迟约 1 秒，但异常写失败时只保证未确认点最多 16 个，不保证时间上限或零丢点；`force-stop` 也不会被描述为普通系统恢复。
...
- M6 已提交为 `19fa99c`：单一 `Channel.UNLIMITED` 事件队列固定先恢复，Service 通过 `LocationEnginePolicy` 与 `LocationEngineSessionController` 区分可见预览、Start 等待和已确认活动行程，并把所有定位生命周期入口收敛为幂等 reconcile；当前开发线又补充了点写失败统一失败流、确认边界重绑 replay、任务移除等待、Start 状态清理竞态、16 点存储背压、普通持久化失败输入阻断和恢复首点分段保护。队列关闭或 actor 异常会唤醒等待者并拒绝新事件；调用方取消等待时，尚未开始消费的命令会被跳过，避免 Activity/Service 销毁后产生迟到写入。通知节流现在把存储阻断状态视为高优先级变化，失败和恢复都会立即刷新。本轮 `gps-core` 54/54、手机版 31/31 JVM（其中控制器 seam 4/4、策略 7/7），AndroidTest 编译、lint、lintVital、Debug/Release、R8、资源压缩和 benchmark 构建完整通过；Pixel_9/API 35 与 API 27 均复验完整 `gps-core` 14/14、手机版 9/9，真实 Service 生命周期 seam 各 3/3；物理低存储、真实 GPS 注册、队列关闭和 `onTaskRemoved()` 尾批回收仍需设备集成验证。
...
- [截图、UI 树与发布产物说明](./artifacts/README.md)：`v0.2.0` Pixel_9 验证文件保存在本地 `artifacts/release-v0.2.0/`；API 35、API 29、API 27 的长测摘要保存在 `artifacts/cargps-mobile-api35-30min-summary.md`、`artifacts/cargps-mobile-api29-30min-summary.md`、`artifacts/cargps-mobile-api27-30min-summary.md`，API 27 整机重启边界摘要为 `artifacts/cargps-mobile-api27-reboot-summary.md`，Room/SQLite 只读写失败摘要为 `artifacts/cargps-mobile-storage-readonly-summary.md`，Runtime/Room 背压链路摘要为 `artifacts/cargps-mobile-runtime-backpressure-summary.md`，这些产物默认不提交 Git。
