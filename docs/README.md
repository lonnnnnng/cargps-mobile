...
- [剩余高风险迁移项](./migration-risks.md)：M1-M7 落地证据、API 27/29 正式旧包升级、跨 API 完整权限矩阵和 Pixel_9 / API 35 30 分钟回归结论，以及 API 27/29 长测、异常环境、最终候选和 AGP 9 的剩余顺序与验收门槛。
...
截图、UI Automator XML、APK 和 `SHA256SUMS` 不放在文档目录，统一归档于本地 [artifacts](../artifacts/README.md)，并按仓库规则默认忽略。当前正式包验证文件名为 `release-v0.2.0/Pixel_9-v0.2.0.png` 和 `Pixel_9-v0.2.0.xml`；API 35 长测摘要为 `cargps-mobile-api35-30min-summary.md`。
