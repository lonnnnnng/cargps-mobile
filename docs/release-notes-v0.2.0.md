...
当前未发布开发线又完成 16 点有界存储背压：磁盘持续不可写时第 17 个点同步拒绝，前台服务停止定位并显示“等待存储恢复”，同时保留活动行程和结束入口；尾批确认后自动清除背压并恢复定位。当前已通过 `gps-core` 51/51、手机版 26/26 JVM、完整构建关卡，以及 Pixel_9 / API 35 与 API 27 各 `gps-core` 12/12、手机版 6/6 instrumentation。上述设备测试未制造真实 `ENOSPC`，因此下一版本仍需完成真实低存储与尾批损失量化、Service 全路径异常竞态、最终候选覆盖升级、Profile 重采集和真实设备长测。当前开发构建仍沿用 `0.2.0 (3)`；“开机自动恢复”也不属于当前已交付能力。最新状态以 [剩余高风险迁移项](./migration-risks.md) 为准。
