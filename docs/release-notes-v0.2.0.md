...
API 27/API 29 已进一步使用公开 `v0.2.0` 正式 APK 创建 SQLite v3 活动行程，再覆盖当前同证书 Release；两端均无损迁移到 Room v4，并恢复“记录中 / 已恢复”、前台服务和单定位线程。API 27/29/31/33 的完整位置权限矩阵已经闭环，API 33 通知拒绝矩阵也已完成；API 31 还覆盖活动行程撤权及 Home/锁屏前台服务短路径。Pixel_9 / API 35 已完成 41 个样本、1831 秒的 Home、系统设置、锁屏和解锁返回回归，结束后历史增加且 Service、通知、定位线程和 GPS 注册正常清理。当前开发构建仍沿用 `0.2.0 (3)`，因此下一版本提升版本号后的最终候选仍需重复覆盖升级。M1-M7 还剩 API 27/API 29 的 30 分钟后台回归、异常环境与真实设备长测，尚未发布新版本。最新状态以 [剩余高风险迁移项](./migration-risks.md) 为准。
