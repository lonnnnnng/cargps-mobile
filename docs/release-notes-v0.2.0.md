...
API 27/API 29 已进一步使用公开 `v0.2.0` 正式 APK 创建 SQLite v3 活动行程，再覆盖当前同证书 Release；两端均无损迁移到 Room v4，并恢复“记录中 / 已恢复”、前台服务和单定位线程。API 31 已完成真实 Approximate/Precise、首次/永久拒绝、设置返回、系统定位开关、活动行程撤权及 Home/锁屏前台服务短路径。当前开发构建仍沿用 `0.2.0 (3)`，因此下一版本提升版本号后的最终候选仍需重复覆盖升级。M1-M7 还剩 API 27/29/33 完整位置矩阵、30 分钟后台回归、异常环境与真实设备长测，尚未发布新版本。最新状态以 [剩余高风险迁移项](./migration-risks.md) 为准。
