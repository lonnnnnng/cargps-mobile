...
以上“已知边界”描述的是 `v0.2.0` 正式包，不代表当前未发布开发线。`v0.2.0` 之后已经完成并提交 M1-M6，包括行程确认状态机、定位前台服务、最后确认检查点与进程恢复、Room schema v4、权限状态机和单一事件队列。M7 Baseline/Startup Profile 已在 Pixel_9 重新生成并验证；API 27/29/33 聚焦回归也已完成。

API 27/API 29 已进一步使用公开 `v0.2.0` 正式 APK 创建 SQLite v3 活动行程，再覆盖当前同证书 Release；两端均无损迁移到 Room v4，并恢复“记录中 / 已恢复”、前台服务和单定位线程。当前开发构建仍沿用 `0.2.0 (3)`，因此下一版本提升版本号后的最终候选仍需重复覆盖升级。M1-M7 还剩 API 31、完整权限矩阵、30 分钟后台回归、异常环境与真实设备长测，尚未发布新版本。最新状态以 [剩余高风险迁移项](./migration-risks.md) 为准。
