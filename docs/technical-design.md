...
- 有效点按最多 16 点或 1 秒组成批次，在单个事务中写入；暂停、结束、读取、已执行的生命周期检查点和正常关闭前强制冲刷尾批次。队列用原子计数预留未确认点名额，磁盘持续不可写时最多保留 16 点，第 17 个点通过 `TripStorageBackpressureException` 同步拒绝，避免定位回调无限堆积内存。正常情况下批次延迟约 1 秒；异常写失败时只保证未确认点数量上限，不保证时间上限。`TripRecordingService.onTaskRemoved()` 以 `CoroutineStart.UNDISPATCHED` 在回调返回前同步入队 Checkpoint，并使用 `KEEP_QUEUED` 保证 Service 销毁或等待协程取消后仍由 Runtime actor 继续冲刷；若整个进程在完成前被系统回收，仍只能按最近确认边界恢复。本轮 probe-only 阻塞探针已把该边界设备化：在真实 Room 批次提交前杀进程时，16 个未确认点全部丢失，不能把 Checkpoint 的尽力冲刷写成零丢点承诺。
...
- 升级：历史证据使用公开 `v0.2.0` 正式 APK 生成真实 SQLite v3 活动行程，再用同证书候选 APK 覆盖安装，验证 Room v4 迁移、活动状态、点数、距离、确认边界和前台服务恢复。`v0.3.0` 已提升到 `0.3.0 (4)`；API 27/API 29 专用镜像清理后本次不重新下载复跑最终版本号覆盖升级，发布说明必须将其列为未复验边界。
- 性能：Baseline Profile 以两个独立场景采集：首屏启动进入 Baseline 与 Startup Profile，完整行程场景覆盖开始、Home、Activity/Service 重绑、暂停、继续和结束。Macrobenchmark 在同一 Pixel_9 上同时记录无预编译与强制 Baseline Profile 的冷启动 TTID；模拟器结果只用于相对比较。当前热路径已重新生成 50,670 行 Baseline Profile 与 49,461 行 Startup Profile，并命中 `LocationEngineSessionController`、`DashboardRuntime`、`TripSessionEventQueue` 和 `TripRecordingService`；旧 ViewModel 与 probe-only 探针类名保持 0。两轮中位改善为 17.46% 和 24.37%，但 CPU 未锁定且第二轮波动明显，不能外推真机绝对性能或宣称整体启动无回退。后续启动、Service、Runtime、队列或存储热路径变化时必须重新生成并复核 APK 内嵌 Profile。
...
M5 跨 API 权限验收已经完成，三档 API 的 30 分钟后台回归和 API 27 整机重启边界也已通过。本轮又补充了点写统一失败流、确认边界 replay、任务移除等待、Start 请求清理顺序、16 点存储背压、普通持久化失败输入阻断、恢复首点分段保护、两类队列取消语义、Service 定位会话控制器、End 前后尾点顺序和 actor 单次自动恢复；Room/SQLite 实际连接只读写失败与受控真实 `SQLiteFullException` 在 API 35/API 27 的存储类各 14/14 通过，Runtime/Room 类各 2/2，当前 `gps-core` instrumentation 合计各 16/16，并通过 `gps-core` 58/58、手机版 33/33 JVM（Runtime 持久化 11/11、控制器 4/4、启动恢复策略 6/6、事件队列 7/7）和完整本地构建。真实 Service 8/8 seam 已覆盖 Start 门禁、可见性重绑、活动行程 Activity 重建重绑、注入式可恢复故障通知/定位编排、已排队 Checkpoint 跨 Service 销毁继续完成、End 前后定位顺序、actor 单次恢复和第二次终态失败；M3 probe-only 探针在两档设备量化了 Checkpoint 提交前进程回收的 16 点损失窗口，M6 外部脚本又闭环前台 Activity/Service 同进程回收、Service 独立恢复和用户返回重绑。M7 已按当前热路径重采集 Baseline/Startup Profile，并完成两轮 Pixel_9 相对冷启动对照。受控页上限仍不等价于物理磁盘 `ENOSPC`；物理低存储和真实系统 GPS 停止/恢复作为已接受残余风险披露，API 27/API 29 最终版本号覆盖升级与真机长测作为本次未复验发布边界。`v0.3.0` 签名 Release、本地门禁、Profile 资产校验和 Pixel_9 安装验证均已完成。后续若热路径变化，Profile 自动重新进入候选门禁。开机自动恢复属于尚未承诺的独立产品决策，不能只添加 receiver 解决，也不能把这些问题收缩成单一“权限补丁”或“升版本”。完整优先级、依赖关系和验收门槛见 [剩余高风险迁移项](./migration-risks.md)。

M1 已建立可确认的行程状态，并补充同步点写失败不终止会话的失败流、恢复首点分段保护和受控真实 `SQLITE_FULL` 回归；M2 已把定位所有权迁入前台服务；M3 已建立最后确认检查点、16 点未确认上限和 `START_STICKY` 恢复门禁，确认边界支持晚到订阅者读取，任务移除路径可把 Checkpoint 同步入队并跨 Service 销毁继续确认，同时验证 API 27 整机重启后需用户打开应用恢复；M4 已完成 Room schema v4、旧版本显式迁移、损坏状态护栏，以及 API 27/API 29 的公开正式旧包覆盖升级。M5 已通过 Pixel_9 / API 35 和 API 27/29/31/33 的完整位置权限矩阵，API 33 通知拒绝矩阵也已验收；M6 单一事件队列、统一 `LocationEngineSessionController`、背压策略、End 前后尾点顺序、活动行程 Activity 重建重绑、actor 第二次终态失败、Checkpoint 提交前进程回收和前台 Activity/Service 同进程回收后的用户返回重绑，已通过 JVM、本地构建、两档设备当前 `gps-core` 16/16 与手机版最近完整回归 14/14 instrumentation，以及 Runtime/Room 两项、Service 生命周期 seam 8/8 和两条外部 probe 脚本；三档 API 还通过了完整 30 分钟常规后台回归。M7 当前热路径 Profile 已刷新并复跑两轮冷启动对照。`v0.3.0` 已完成签名 Release、本地完整关卡、Profile 资产复核和 Pixel_9 最终验收；物理低存储、真实系统 GPS、API 27/API 29 最终版本号覆盖升级与真机 2 小时长测按未验证边界披露。若后续热路径发生变化，重新生成 Profile；若产品决定支持开机自动恢复，另开迁移项评审 `BOOT_COMPLETED` 与后台位置权限；M8 AGP 9 继续独立延后。
