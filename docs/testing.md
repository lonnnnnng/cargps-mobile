...
更新日期：2026-08-08 12:26:17（北京时间，UTC+8）
...
`TripSessionCoordinatorTest` 通过 fake storage 注入开始、暂停、恢复、结束、定位点同步写入失败、存储背压和异步写入错误，验证元数据命令失败时保持上一个确认模式、点写失败不终止会话、背压恢复后清除临时错误、重复命令不重复写、多段暂停只扣除真实暂停时长。`QueuedTripStorageTest` 另外验证瞬时批量失败重试成功不误报、连续失败由屏障抛出、晚到订阅者仍可取得最近确认检查点、未确认点最多 16 个且第 17 个被拒绝，以及存储恢复后配额释放并可继续写入。`TripStorageFailureIntegrationTest` 组合真实 `TripSessionCoordinator + QueuedTripStorage`，断言永久批次失败期间保持最后确认检查点，恢复后再确认原尾批；这些都是 JVM 故障注入，不等价于 Room/SQLite 的真实 `ENOSPC`。

M6 当前开发线的 `TripSessionEventQueueTest` 5/5，覆盖 Restore 固定优先与完整 FIFO、等待型 Start、关闭取消、actor 异常终止和消费时连续 Toggle。协调器测试补充 End 前尾点纳入、End 后点拒绝、点写失败统一失败流、16 点背压和确认恢复；Runtime 测试确认等待型 Start 返回前已经同步发布记录状态，并确认任务移除检查点在存储确认后才返回。手机版 `TripStartOrchestratorTest` 5/5，覆盖纯编排回调在确认前不调用 `startLocation`、最终状态先于请求标记清理、Start 失败、权限等待期间失效和异常清理；`LocationEnginePolicyTest` 6/6 进一步覆盖存储背压时无论 Activity 是否可见都停止定位。
...
- 本地 JVM 完整测试：`gps-core` 51/51、`mobile-app` 26/26 通过，失败与错误均为 0。
- 本地完整构建关卡：AndroidTest 编译、`lintDebug`、`lintVitalRelease`、Debug/Release、R8、资源压缩和 `baselineprofile:assembleBenchmarkRelease` 全部成功；Gradle 汇总为 `BUILD SUCCESSFUL`，261 项任务中 53 项执行、208 项复用缓存。
- Pixel_9 复验：确认 `emulator-5554` 的 `ro.boot.qemu.avd_name = Pixel_9` 后显式设置 `ANDROID_SERIAL=emulator-5554`；`gps-core` 12/12、`mobile-app` 6/6 instrumentation 通过。
- Android 8.1 / API 27 复验：确认 `emulator-5556` 为 API 27、1024x600、160 dpi，Gradle 识别为 `CASKA_1024x600(AVD) - 8.1.0`；显式锁定该 serial 后 `gps-core` 12/12、`mobile-app` 6/6 instrumentation 通过。两档设备关卡都只证明既有 instrumentation 未回退，不替代低存储故障注入。
...
- 未完成证据：尚未在 Android 设备上让 Room/SQLite 产生真实 `ENOSPC` 或永久写失败，也没有断言“等待存储恢复”通知、GPS 注册停止、磁盘恢复耗时和定位自动重启；当前不能宣称真实低存储零丢点。
- 未完成分段证据：当前测试断言被拒绝点不进入累计器和确认检查点，但尚未断言恢复首个有效点会断开被拒绝点前后的定位段；需要补距离、速度和首点 sequence 的设备/Runtime 断言。
...
- 本地完整关卡此前为 `gps-core` 44/44、手机版 24/24 JVM；当前背压加固后为 `gps-core` 51/51、手机版 26/26 JVM，AndroidTest 编译、`lintDebug`、`lintVitalRelease`、Debug/Release、R8、资源压缩和 `baselineprofile:assembleBenchmarkRelease` 均成功。
- Pixel_9 / API 35 与 Android 8.1 / API 27 instrumentation：本轮两端均重新执行 `gps-core` 12/12、手机版 6/6。设备通过 serial 与 AVD/API/分辨率明确确认，所有 Gradle 设备任务通过 `ANDROID_SERIAL` 锁定，未操作 Redmi。
...
- API 27/API 29 的聚焦 `START_STICKY` 恢复已通过；Pixel_9 / API 35、Android 10 / API 29 与 Android 8.1 / API 27 的完整 30 分钟 Home/切换应用/锁屏已通过；API 27 整机重启边界也已验证为“不开机自动拉起，打开应用后恢复”。真实低存储、尾批损失量化、Service 全路径异常竞态和真实设备 2 小时长测仍未完成。
...
Pixel_9 / API 35、Android 10 / API 29 与 Android 8.1 / API 27 的 30 分钟后台记录已经完成，API 27 整机重启边界也已完成验证；最新背压代码已通过 JVM、完整本地构建和 Pixel_9/API 27 既有 instrumentation。尚未完成的真实低存储、尾批损失量化、Service 全路径异常竞态、Profile 重采集、最终候选版本号与签名升级复验和真实设备长测见 [剩余高风险迁移项](./migration-risks.md)，不能用常规长测替代异常环境门槛。当前不支持的“开机自动恢复”另需产品决策和权限迁移评审。
