...
| M2 | P0 | API 27/29/35 30 分钟已验证，待异常环境/发版 | 定位前台服务 | 设备重启、低存储和真实道路长时行为仍未验收 | 已覆盖最低系统可靠后台记录 |
...
| M6 | P1；阻断发版 | API 27/29/35 30 分钟已验证；异常环境与真机待验收 | 生命周期并发与真机长测 | 不可见 Start 等待已禁止后台定位；设备重启、低存储、尾批量化和真实设备仍未完成 | 最后定位点必不丢失或已验证真实道路和长时功耗 |
...
M1-M6 的核心实现已经通过本地关卡、Pixel_9 / API 35，以及 API 27/29/31/33 聚焦验证；M5 跨 API 完整权限矩阵已闭环，M7 性能资产已刷新，API 27/API 29 的公开正式旧包覆盖升级也已完成。API 27/29/35 的 30 分钟后台回归已经全部通过，但异常环境、真实设备和最终候选身份仍未完成，因此还不能直接发版。发布候选至少还要完成以下三组关卡：
...
Room schema 与显式迁移实现已经完成，API 27/29/33 上的 12 个 fixture/事务/损坏门禁 instrumentation 均已通过；API 27/API 29 的公开正式旧包升级也已保留真实数据库证据。API 27/29/35 的 30 分钟结果已全部通过，剩余发布风险集中为设备重启、低存储、尾批损失量化、最终版本号候选是否仍保持同一升级结果，以及真实设备 2 小时长测。AGP 9 仍需分阶段推进，不能与异常恢复和最终候选复验塞入同一次架构改动。真实设备测试必须取得单独设备授权；当前模拟器授权不能自动扩展到 Redmi。
...
未完成门槛：三档模拟器 30 分钟门禁已通过；仍需验证设备重启、低存储、异常终止前后的尾批损失、冷启动恢复首点、Activity 重建与服务重连竞态。2026-08-07 被外部 `force-stop` 中断的旧监测仍然无效，不与后续成功结果合并计数。
...
- 冷启动恢复与首点、Activity 重建、服务重连和 `onTaskRemoved()` 并发仍需在三档 API 的异常场景中复核；30 分钟常规后台长测已通过，但没有注入这些异常竞态。
...
下一发布候选必须先通过第 3 节剩余异常与身份关卡，并完成取得单独设备授权后的真实设备 2 小时长测。即使通过，发布说明仍必须保留以下边界：API 27/29/35 的 30 分钟结果不代表设备重启、低存储或真机；模拟器性能不代表真机；事件队列只保证单进程内顺序；恢复上限是最后确认检查点而非零丢点；`force-stop`、断电、设备重启和低存储不属于当前已验证恢复结论；当前已验证 API 27/API 29 的公开正式旧包迁移和 API 27/29/31/33/35 完整权限矩阵，但最终提升版本号后的候选仍未完成。AGP 9 继续延后，不与异常恢复和最终候选门禁合并。
