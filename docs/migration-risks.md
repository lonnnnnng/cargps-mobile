...
更新时间：2026-08-07 21:35:47（北京时间，UTC+8）

## 1. 当前基线

`v0.2.0` 已完成依赖升级、增量行程统计、定位回调线程化、SQLite 批量写入、R8、Baseline Profile 和 Pixel_9 发布验证。`v0.2.0` 之后已完成 M1 行程确认状态机、M2 定位前台服务、M3 最后确认检查点和进程重建恢复门禁，以及 M4 Room schema v4、旧版本显式迁移和数据损坏护栏；四项均尚未随新版本发布。当前继续保留 `minSdk = 27`。
...
| M5 | P1；阻断发版 | 待实现 | 权限状态机 | 仅接受精确定位，近似、永久拒绝、系统定位与通知拒绝未闭环 | 所有 Android 权限分支可用或后台记录一定可见 |
...
M1-M4 的核心实现已经通过 Pixel_9 / API 35 聚焦验证，但还不能直接发版。发布候选构建前至少完成以下四组关卡：
...
当前缺口：Activity、Service 和 `LocationEngine` 只以 `ACCESS_FINE_LOCATION` 成功与否作主要判断。仅近似、首次拒绝、永久拒绝、系统定位关闭、通知拒绝和设置返回尚未形成统一状态；拒绝后启动意图也可能残留，错误分支缺少对应设置入口。

改造边界：

- 区分精确、近似、拒绝、永久拒绝、系统定位关闭和从设置返回后的状态。
- 明确产品策略：若近似定位不足以支持速度和里程，应解释原因并引导精确授权，而不是循环弹窗。
- 前台服务所需位置权限必须在启动前完成；Android 13 及以上的通知权限拒绝场景也要提供可解释状态，不能让用户误判后台记录是否正在运行。
- 后台位置权限不是 M2 首版前台服务的默认前置条件，只有确需从后台创建定位服务时才单独申请和验证。

发布前最小门槛：覆盖精确定位拒绝、永久拒绝、系统定位关闭，以及 Android 13+ 通知拒绝；任何失败分支都不崩溃、不循环请求、不错误显示“正在记录”，通知不可见时也不能让用户误判后台记录状态。
...
M1-M6 的发布候选必须先通过第 3 节四组阻断关卡。即使通过，发布说明仍必须保留以下边界：模拟器性能不代表真机；恢复上限是最后确认检查点而非零丢点；`force-stop`、断电、设备重启和低存储不属于当前已验证恢复结论；Room 迁移已完成但跨 API 升级安装、全部权限分支和真实道路长测仍未完成。
