...
| M2 | P0 | API 35/29 30 分钟已验证；API 27 待长测/发版 | 定位前台服务 | API 27 的完整 30 分钟 Home、切换应用和锁屏仍未验收 | 已覆盖最低系统可靠后台记录 |
...
| M6 | P1；阻断发版 | API 35/29 30 分钟已验证；API 27 与真机待验收 | 生命周期并发与真机长测 | 不可见 Start 等待已禁止后台定位；API 27 长测和真实设备仍未完成 | 最后定位点必不丢失或已验证真实道路和长时功耗 |
...
M1-M6 的核心实现已经通过本地关卡、Pixel_9 / API 35，以及 API 27/29/31/33 聚焦验证；M5 跨 API 完整权限矩阵已闭环，M7 性能资产已刷新，API 27/API 29 的公开正式旧包覆盖升级也已完成。Pixel_9 / API 35 与 Android 10 / API 29 的 30 分钟后台回归已经通过，但 API 27 长测、异常环境和最终候选身份仍未完成，因此还不能直接发版。发布候选至少还要完成以下三组关卡：

1. **跨 API 运行时**：Pixel_9 / API 35 已完成 41 个样本、1831 秒回归，Android 10 / API 29 已完成 41 个样本、1816 秒回归；API 27 的活动行程 `START_STICKY` 聚焦恢复已通过，但仍需在最低系统环境完成同等 30 分钟 Home、切换应用、锁屏和停止回归。
...
Room schema 与显式迁移实现已经完成，API 27/29/33 上的 12 个 fixture/事务/损坏门禁 instrumentation 均已通过；API 27/API 29 的公开正式旧包升级也已保留真实数据库证据。API 35 与 API 29 的 30 分钟结果进一步收敛了当前前台服务风险，但不能替代 API 27 或真机结论。剩余发布风险集中为 API 27 长测、异常恢复边界和最终版本号候选是否仍保持同一升级结果。真实设备 2 小时长测和 AGP 9 仍需分阶段推进，不能与长测、异常恢复和最终候选复验塞入同一次架构改动。真实设备测试必须取得单独设备授权；当前模拟器授权不能自动扩展到 Redmi。
...
验证证据：Pixel_9 / API 35 上 SQLite instrumentation 4/4、`mobile-app` instrumentation 3/3；运行时服务为 `location` 前台类型，通知和结束操作可用。Home、锁屏、Activity 重建、重复打开、覆盖安装触发进程重建和通知结束短路径通过，定位线程始终为 1，crash buffer 为空。2026-08-08 又完成 41 个样本、1831 秒的 API 35 完整后台回归，以及 41 个样本、1816 秒的 API 29 完整后台回归；两轮 Home、系统设置、锁屏和解锁返回期间 PID 均未变化、前台服务持续、定位线程始终为 1、crash buffer 始终为 0，正常结束后历史增加且资源归零。API 29 的活动 GPS 注册始终为 1，结束后为 0、`mStarted=false`。同日 API 27 与 API 29 分别通过 `gps-core` 12/12、手机版 6/6 instrumentation；两者均完成开始、Home、单定位线程、进程恢复、结束和 Home 后资源清理。证据位于本地 `artifacts/cargps-mobile-api35-30min-*`、`artifacts/cargps-mobile-api29-30min-*`、`artifacts/cargps-mobile-api27-*`。

未完成门槛：API 27 上执行与 API 35/API 29 同等的完整 30 分钟 Home、切换应用和锁屏；通知持续可见，轨迹连续，停止后无回调，重建 Activity 不产生第二个定位监听。2026-08-07 被外部 `force-stop` 中断的旧监测仍然无效，不与后续成功结果合并计数。
...
- Pixel_9 / API 35：`gps-core` instrumentation 12/12、手机版 6/6；开始后为 `location` 前台服务，应用 UID 连续两次 ensure 后定位线程仍为 1，结束后历史增加一段，Home 后 Service 清除，crash buffer 无 CarGPS 记录。随后完整 30 分钟路径也通过：41 个样本、1831 秒内同一 PID、前台服务、通知和单定位线程持续；锁屏睡眠不丢失运行状态，结束后资源与 GPS 注册清理。Android 10 / API 29 的 41 个样本、1816 秒同级路径也通过，活动 GPS 注册始终为 1，结束后为 0。
...
- 冷启动恢复与首点、Activity 重建、服务重连和 `onTaskRemoved()` 并发仍需在 API 27 的完整 30 分钟关卡及后续异常场景中复核；API 35/API 29 已通过常规后台长测，但没有注入这些异常竞态。
...
下一发布候选必须先通过第 3 节三组阻断关卡，并完成取得单独设备授权后的真实设备 2 小时长测。即使通过，发布说明仍必须保留以下边界：API 35/API 29 的 30 分钟结果不代表 API 27 或真机；模拟器性能不代表真机；事件队列只保证单进程内顺序；恢复上限是最后确认检查点而非零丢点；`force-stop`、断电、设备重启和低存储不属于当前已验证恢复结论；当前已验证 API 27/API 29 的公开正式旧包迁移和 API 27/29/31/33/35 完整权限矩阵，但最终提升版本号后的候选仍未完成。AGP 9 继续延后，不与本轮运行时门禁合并。
