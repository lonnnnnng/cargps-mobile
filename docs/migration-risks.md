...
更新时间：2026-08-08 08:47:44（北京时间，UTC+8）

## 1. 当前基线

`v0.2.0` 已完成依赖升级、增量行程统计、定位回调线程化、SQLite 批量写入、R8、Baseline Profile 和 Pixel_9 发布验证。`v0.2.0` 之后的 M1-M6 已完成当前开发线的核心实现并提交；M7 Baseline/Startup Profile 已在 Pixel_9 重新生成并完成两轮冷启动对照。2026-08-08 又完成 API 27、API 29、API 31、API 33 的聚焦 instrumentation、运行时回归和完整位置权限矩阵，在 API 27/API 29 完成公开正式 `v0.2.0` 到当前同证书 Release 的 SQLite v3 -> Room v4 覆盖升级，并在 Pixel_9 / API 35 完成 41 个样本、1831 秒的后台回归。M1-M7 尚未随新版本发布，API 27/API 29 的完整 30 分钟、真实设备和异常环境长测仍未完成；当前开发构建也仍沿用 `0.2.0 (3)`。当前继续保留 `minSdk = 27`。

M2 已消除“Activity 退到后台就主动停止定位”的旧结构。API 27/API 29 已验证开始、Home、单定位线程、普通进程 `SIGKILL` 后 `START_STICKY` 恢复、结束和资源清理；API 31 已验证开始、Home、锁屏、单定位线程、活动行程撤权后的受阻结束和资源清理，但未执行普通进程恢复。Pixel_9 / API 35 已完成 Home、系统设置、锁屏睡眠和解锁返回的完整 30 分钟监测，期间同一 PID、前台服务、活动通知和单定位线程持续，正常结束后资源完整清理。API 27/API 29 仍只有聚焦短路径，不能扩展为跨版本长时后台可靠性结论。
...
| M2 | P0 | API 35 30 分钟已验证；API 27/29 待长测/发版 | 定位前台服务 | API 27/API 29 的完整 30 分钟 Home、切换应用和锁屏仍未验收 | 已跨 Android 版本可靠后台记录 |
...
| M6 | P1；阻断发版 | API 35 30 分钟已验证；API 27/29 与真机待验收 | 生命周期并发与真机长测 | 不可见 Start 等待已禁止后台定位；API 27/29 长测和真实设备仍未完成 | 最后定位点必不丢失或已验证真实道路和长时功耗 |
...
M1-M6 的核心实现已经通过本地关卡、Pixel_9 / API 35，以及 API 27/29/31/33 聚焦验证；M5 跨 API 完整权限矩阵已闭环，M7 性能资产已刷新，API 27/API 29 的公开正式旧包覆盖升级也已完成。Pixel_9 / API 35 的 30 分钟后台回归已经通过，但 API 27/API 29 长测、异常环境和最终候选身份仍未完成，因此还不能直接发版。发布候选至少还要完成以下三组关卡：

1. **跨 API 运行时**：Pixel_9 / API 35 已完成 41 个样本、1831 秒的 Home、系统设置、锁屏和解锁返回回归；API 27/API 29 的活动行程 `START_STICKY` 聚焦恢复已通过，但仍需在这两个环境完成同等 30 分钟 Home、切换应用、锁屏和停止回归。
...
Room schema 与显式迁移实现已经完成，API 27/29/33 上的 12 个 fixture/事务/损坏门禁 instrumentation 均已通过；API 27/API 29 的公开正式旧包升级也已保留真实数据库证据。Pixel_9 / API 35 的 30 分钟结果进一步收敛了当前前台服务风险，但不能替代 API 27/API 29 或真机结论。剩余发布风险集中为 API 27/API 29 长测、异常恢复边界和最终版本号候选是否仍保持同一升级结果。真实设备 2 小时长测和 AGP 9 仍需分阶段推进，不能与长测、异常恢复和最终候选复验塞入同一次架构改动。真实设备测试必须取得单独设备授权；当前模拟器授权不能自动扩展到 Redmi。
...
验证证据：Pixel_9 / API 35 上 SQLite instrumentation 4/4、`mobile-app` instrumentation 3/3；运行时服务为 `location` 前台类型，通知和结束操作可用。Home、锁屏、Activity 重建、重复打开、覆盖安装触发进程重建和通知结束短路径通过，定位线程始终为 1，crash buffer 为空。2026-08-08 又完成 41 个样本、1831 秒的完整后台回归：Home、系统设置、锁屏 `Dozing/Asleep` 和解锁返回期间 PID 始终为 `4395`、前台服务持续、定位线程始终为 1、crash buffer 始终为 0；正常结束后历史增加，Home 后 Service、活动通知和定位线程归零，GPS provider 为 `OFF`。同日 API 27 与 API 29 分别通过 `gps-core` 12/12、手机版 6/6 instrumentation；两者均完成开始、Home、单定位线程、进程恢复、结束和 Home 后资源清理。证据位于本地 `artifacts/cargps-mobile-api35-30min-*`、`artifacts/cargps-mobile-api27-*`、`artifacts/cargps-mobile-api29-*`。

未完成门槛：API 27 和 API 29 上分别执行与 API 35 同等的完整 30 分钟 Home、切换应用和锁屏；通知持续可见，轨迹连续，停止后无回调，重建 Activity 不产生第二个定位监听。2026-08-07 被外部 `force-stop` 中断的旧监测仍然无效，不与本次 API 35 成功结果合并计数。
...
- Pixel_9 / API 35：`gps-core` instrumentation 12/12、手机版 6/6；开始后为 `location` 前台服务，应用 UID 连续两次 ensure 后定位线程仍为 1，结束后历史增加一段，Home 后 Service 清除，crash buffer 无 CarGPS 记录。随后完整 30 分钟路径也通过：41 个样本、1831 秒内同一 PID、前台服务、通知和单定位线程持续；锁屏睡眠不丢失运行状态，结束后资源与 GPS 注册清理。
...
- 冷启动恢复与首点、Activity 重建、服务重连和 `onTaskRemoved()` 并发仍需在 API 27/API 29 的完整 30 分钟关卡中复核；API 35 已通过常规后台长测，但没有注入这些异常竞态。
...
下一发布候选必须先通过第 3 节三组阻断关卡，并完成取得单独设备授权后的真实设备 2 小时长测。即使通过，发布说明仍必须保留以下边界：Pixel_9 / API 35 的 30 分钟结果不代表 API 27/API 29 或真机；模拟器性能不代表真机；事件队列只保证单进程内顺序；恢复上限是最后确认检查点而非零丢点；`force-stop`、断电、设备重启和低存储不属于当前已验证恢复结论；当前已验证 API 27/API 29 的公开正式旧包迁移和 API 27/29/31/33/35 完整权限矩阵，但最终提升版本号后的候选仍未完成。AGP 9 继续延后，不与本轮运行时门禁合并。
