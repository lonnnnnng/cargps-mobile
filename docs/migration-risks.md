...
- **未验证（发布阻断）**：物理磁盘 `ENOSPC`/低存储下的通知与真实 GPS 注册停止/恢复、最终候选版本号升级复验、热路径 Profile 重采集和真实设备长测。
