...
- **未验证（发布阻断）**：物理磁盘 `ENOSPC`/低存储下的通知与真实 GPS 注册停止/恢复、Activity 与 Service 进程同时复杂重建、最终候选版本号升级复验、热路径 Profile 重采集和真实设备长测。
...
| M6 | P1；阻断发版 | API 27/29/35 30 分钟、本地完整关卡、Pixel_9/API 27 完整套件、真实 Service 8/8 seam 及 M3 提交前进程回收探针已通过；异常环境、低存储与真机待验收 | 生命周期并发与真机长测 | 不可见 Start 等待已禁止后台定位；控制器 4/4 JVM 与 Service 生命周期 8/8 设备 seam 已覆盖确认门禁、重绑幂等、活动行程 Activity 重建重绑、注入式可恢复故障、已排队 Checkpoint、End 前后定位顺序、actor 单次恢复和第二次终态失败；另已量化提交前进程回收的 16 点损失窗口，但真实低存储/系统 GPS、Activity 与 Service 进程同时复杂重建和真实设备仍未完成 | 最后定位点必不丢失或已验证真实道路和长时功耗 |
...
2. **低存储与事件顺序**：Room 实际连接的永久只读写失败已经验证存储层原子性（API 35/API 27 存储类各 13/13），Runtime/Room 专项在两档设备各 1/1、完整 `gps-core` 各 14/14 验证前 16 点保留、第 17 点拒绝和恢复后检查点确认；真实 Service 8/8 seam 又验证 Start 确认门禁、可见性重绑、活动行程 Activity 重建重绑、注入式可恢复写失败、任务移除 Checkpoint、End 前后定位顺序、actor 单次恢复和第二次终态失败。M3 probe-only 真实 Room 阻塞探针还验证了 Checkpoint 提交前进程回收，并精确量化 16 点未确认损失窗口。仍需制造物理 `ENOSPC`/低存储并接入真实 Room 与系统 GPS 注册，量化恢复耗时，并补冷启动恢复首点与 Activity/Service 进程同时复杂重建场景。API 27 整机重启已证明当前边界是“用户打开应用后恢复”，不应写成开机自动继续。
...
未完成门槛：三档模拟器 30 分钟门禁、API 27 整机重启边界，以及 Checkpoint 提交前进程回收的 16 点损失量化均已通过；仍需用物理 `ENOSPC` 验证通知、GPS 注册停止、结束入口、恢复耗时和自动恢复定位，并补冷启动恢复首点、Activity 重建与 Service 重连竞态。2026-08-07 被外部 `force-stop` 中断的旧监测仍然无效，不与后续成功结果合并计数。当前手机版不注册 `BOOT_COMPLETED`，因此重启后需用户打开应用恢复；若改变这一边界，另立后台位置权限与系统启动迁移。
...
剩余门槛：Checkpoint 提交前整个进程被回收及 16 点损失窗口量化已经完成；仍需验证物理低存储下的通知、真实 GPS 注册、16 点边界与恢复耗时，以及 Activity/Service 进程同时复杂重建。Room 只读连接测试已覆盖存储层失败保留，但尚未在物理低存储和真实系统 GPS 回调竞态中验收恢复首点分段。`force-stop` 会阻止系统自动启动，整机重启当前也不会自动启动，不能与普通进程回收混为一谈。若未来要支持开机自动恢复，必须单独处理 `BOOT_COMPLETED`、后台位置权限和 Android 14/15 规则。
...
- 现有 Service seam 已覆盖 Start 存储确认、空闲预览的可见性解绑/重绑、活动行程 Activity 重建后重绑原 Service、注入式可恢复写失败、任务移除 Checkpoint、End 前后定位顺序、actor 单次恢复和第二次终态失败；Room/Runtime 专项已覆盖连接级事务失败到 Runtime 背压，M3 probe-only 真实 Room 探针已覆盖 Checkpoint 完成前整个进程被回收并量化 16 点损失窗口。仍缺少 Activity 与 Service 进程同时复杂重建，以及物理低存储与真实系统 GPS 背压/恢复的设备集成断言。
...
- Checkpoint 完成前的进程回收已在 API 35/API 27 通过 probe-only 真实 Room 探针复核；冷启动恢复首点、Activity 与 Service 进程同时复杂重建仍需在异常场景中补测。30 分钟常规后台长测已通过，但没有注入这些剩余竞态。存储层只读失败已通过 API 35/API 27 各 13/13，Runtime/Room 背压专项各 1/1，现有 Service seam 覆盖确认门禁、空闲预览重绑、活动行程 Activity 重建重绑、单次可恢复写失败、End 前后定位顺序、actor 单次恢复、第二次终态失败和 Service 销毁取消，但不能替代物理低存储或上述其余异常路径证据。
