...
- **未验证（发布阻断）**：物理磁盘 `ENOSPC`/低存储下的通知与真实 GPS 注册停止/恢复、Activity 与 Service 进程同时复杂重建、Checkpoint 完成前整个进程被回收、尾批损失量化、最终候选版本号升级复验、热路径 Profile 重采集和真实设备长测。
...
| M6 | P1；阻断发版 | API 27/29/35 30 分钟、本地完整关卡、Pixel_9/API 27 完整套件及真实 Service 8/8 seam 已通过；异常环境、低存储与真机待验收 | 生命周期并发与真机长测 | 不可见 Start 等待已禁止后台定位；控制器 4/4 JVM 与 Service 生命周期 8/8 设备 seam 已覆盖确认门禁、重绑幂等、活动行程 Activity 重建重绑、注入式可恢复故障、已排队 Checkpoint、End 前后定位顺序、actor 单次恢复和第二次终态失败，但 Checkpoint 完成前的进程回收、真实低存储/系统 GPS、Activity 与 Service 进程同时复杂重建、尾批量化和真实设备仍未完成 | 最后定位点必不丢失或已验证真实道路和长时功耗 |
...
2. **低存储与事件顺序**：Room 实际连接的永久只读写失败已经验证存储层原子性（API 35/API 27 存储类各 13/13），Runtime/Room 专项在两档设备各 1/1、完整 `gps-core` 各 14/14 验证前 16 点保留、第 17 点拒绝和恢复后检查点确认；真实 Service 8/8 seam 又验证 Start 确认门禁、可见性重绑、活动行程 Activity 重建重绑、注入式可恢复写失败、任务移除 Checkpoint、End 前后定位顺序、actor 单次恢复和第二次终态失败。仍需制造物理 `ENOSPC`/低存储并接入真实 Room 与系统 GPS 注册，量化恢复耗时和进程终止前后的尾批损失，并补冷启动恢复首点、Activity 与 Service 进程同时复杂重建和 Checkpoint 完成前整个进程被回收的设备场景。API 27 整机重启已证明当前边界是“用户打开应用后恢复”，不应写成开机自动继续。
...
- 现有 Service seam 已覆盖 Start 存储确认、空闲预览的可见性解绑/重绑、活动行程 Activity 重建后重绑原 Service、注入式可恢复写失败、任务移除 Checkpoint、End 前后定位顺序、actor 单次恢复和第二次终态失败；Room/Runtime 专项已覆盖连接级事务失败到 Runtime 背压。仍缺少 Activity 与 Service 进程同时复杂重建、物理低存储与真实系统 GPS 背压/恢复，以及 Checkpoint 完成前整个进程被回收的设备集成断言。
...
- 冷启动恢复与首点、Activity 与 Service 进程同时复杂重建和 Checkpoint 完成前的进程回收仍需在三档 API 的异常场景中复核；30 分钟常规后台长测已通过，但没有注入这些异常竞态。存储层只读失败已通过 API 35/API 27 各 13/13，Runtime/Room 背压专项各 1/1，现有 Service seam 覆盖确认门禁、空闲预览重绑、活动行程 Activity 重建重绑、单次可恢复写失败、End 前后定位顺序、actor 单次恢复、第二次终态失败和 Service 销毁取消，但不能替代物理低存储或上述其余异常路径证据。
