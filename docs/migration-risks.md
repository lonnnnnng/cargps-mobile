...
- **未验证（发布阻断）**：物理磁盘 `ENOSPC`/低存储、通知“等待存储恢复”、GPS 注册停止与恢复、Service 全路径竞态、尾批损失量化、最终候选版本号升级复验、热路径 Profile 重采集和真实设备长测。
...
| --- | --- | --- | --- | --- | --- |
| M1 | P0 | 核心、16 点背压、恢复首点 JVM 与 Room/Runtime 连接级故障已验证，待 Service 故障注入/发版 | 行程状态写入确认与单一所有者 | 命令已串行并等待存储确认；同步点写失败、背压和定位分段断点进入统一失败流；连接级故障已保留确认边界；未承诺跨进程 exactly-once 命令语义 | 已在正式版本交付、物理低存储无损或断电零丢点 |
| M2 | P0 | 常规 API 27/29/35 30 分钟与 API 27 重启边界已验证，待物理低存储/发版 | 定位前台服务 | 背压停止/恢复策略已实现并通过普通 instrumentation；Room 只读故障只覆盖存储层，真实低存储通知、GPS 停止/恢复、厂商电源管理和真实道路长时行为仍未验收；当前不支持开机自动拉起 | 已覆盖物理低存储、所有厂商后台场景或开机自动恢复 |
...
| M6 | P1；阻断发版 | API 27/29/35 30 分钟、本地完整关卡和 Pixel_9/API 27 复验已通过；真实 Service 异常竞态、低存储与真机待验收 | 生命周期并发与真机长测 | 不可见 Start 等待已禁止后台定位；`LocationEngineSessionController` 已统一启停并通过 4/4 JVM seam，Start 状态清理、任务移除等待和背压策略已有 seam，但 Service 全路径重连、真实低存储、尾批量化和真实设备仍未完成 | 最后定位点必不丢失或已验证真实道路和长时功耗 |
...
M1-M6 的核心实现已经通过本地关卡、Pixel_9 / API 35，以及 API 27/29/31/33 聚焦验证；最新 16 点背压加固后又通过完整本地构建，并在 Pixel_9 / API 35 与 API 27 各完成 20 项当前 instrumentation（`gps-core` 14/14 + 手机版 6/6）。M5 跨 API 完整权限矩阵已闭环，API 27/API 29 的公开正式旧包覆盖升级也已完成。API 27/29/35 的 30 分钟后台回归和 API 27 整机重启边界已经通过，但物理低存储、Service 异常环境、最终候选身份、真实设备和热路径 Profile 重采集仍未完成，因此还不能直接发版。发布候选至少还要完成以下四组关卡：
...
2. **低存储与事件顺序**：Room 实际连接的永久只读写失败已经验证存储层原子性（API 35/API 27 存储类各 13/13），新增 Runtime/Room 专项又在两档设备各 1/1、完整 `gps-core` 各 14/14 验证前 16 点保留、第 17 点拒绝和恢复后检查点确认；但仍需制造物理 `ENOSPC`/低存储并接入完整 `TripRecordingService` 路径，断言通知显示“等待存储恢复”、GPS 注册停止、结束入口保留、磁盘恢复后定位自动恢复和耗时；继续量化进程终止前后的尾批损失，并补冷启动恢复首点、Activity 重建、Service 重连、队列关闭和 `onTaskRemoved()` 并发。API 27 整机重启已证明当前边界是“用户打开应用后恢复”，不应写成开机自动继续。
...
P0 低存储项仍包括设备级恢复首点的定位分段校验：Room/SQLite 只读连接测试和 Runtime/Room 专项已证明连接级失败不会清除已确认数据，前 16 点可保留并在恢复后确认，JVM `DashboardRuntimePersistenceTest` 已证明被拒绝样本不会成为恢复后第一点的连接基准；但物理 `ENOSPC`、Service 回调竞态和 GPS 停止/恢复仍需设备证据。
...
- 2026-08-08 最新背压、普通持久化失败输入阻断与恢复首点分段加固后 `gps-core` 53/53、手机版 31/31 JVM 通过；AndroidTest 编译、`lintDebug`、`lintVitalRelease`、Debug/Release、R8、资源压缩和 `baselineprofile:assembleBenchmarkRelease` 完整通过。随后在 `ro.boot.qemu.avd_name = Pixel_9` 的 `emulator-5554 / API 35`，以及 Gradle 识别为 `CASKA_1024x600(AVD) - 8.1.0` 的 `emulator-5556 / API 27` 上各复验完整 `gps-core` 14/14、手机版 6/6 instrumentation；Runtime/Room 背压专项各 1/1 为此前同版本代码的连接级证据，本轮控制器接入后重新执行的是完整套件。serial 不能视为永久映射。
- `TripSessionEventQueueTest` 5/5，覆盖 Restore/FIFO、等待型 Start、关闭取消、actor 异常终止和连续 Toggle；协调器测试确认结束前尾点进入统计、End 后点明确拒绝，Runtime 测试确认等待型 Start 返回前已发布记录状态，并验证普通存储失败与背压恢复首点都不会跨故障窗口补算距离、速度或确认点序列。
...
- Pixel_9 / API 35：既有运行时短路径和本轮 seam 加固后的 instrumentation 均为完整 `gps-core` 14/14、手机版 6/6，Runtime/Room 背压专项 1/1；开始后为 `location` 前台服务，应用 UID 连续两次 ensure 后定位线程仍为 1，结束后历史增加一段，Home 后 Service 清除，crash buffer 无 CarGPS 记录。随后完整 30 分钟路径也通过：41 个样本、1831 秒内同一 PID、前台服务、通知和单定位线程持续；锁屏睡眠不丢失运行状态，结束后资源与 GPS 注册清理。Android 10 / API 29 与 Android 8.1 / API 27 的 41 个样本、1816 秒同级路径也通过，活动 GPS 注册始终为 1，结束后为 0；API 27 的定位线程由 `/proc` 侧车确认始终为 1。
...
- 现有 `TripSessionEventQueueTest`、`TripStartOrchestratorTest`、`LocationEngineSessionControllerTest`、存储失败集成测试、Runtime 检查点等待测试和 `LocationEnginePolicyTest` 仍主要覆盖 JVM seam；Room/Runtime 专项已覆盖连接级事务失败到 Runtime 背压，但不等价于真实 Service 全路径。仍缺少 Start 等待期间 Activity 重建/重绑、队列关闭或 actor 异常、End 与并发尾点、物理低存储背压/恢复、`onTaskRemoved()` 请求尚未完成即回收的设备集成断言。
...
- 冷启动恢复与首点、Activity 重建、Service 重连和 `onTaskRemoved()` 并发仍需在三档 API 的异常场景中复核；30 分钟常规后台长测已通过，但没有注入这些异常竞态。存储层只读失败已通过 API 35/API 27 各 13/13，Runtime/Room 背压专项各 1/1，不能替代上述 Service 级证据。
