...
- **已验证（真实 Service 生命周期 seam）**：`TripRecordingServiceLifecycleInstrumentedTest` 在 Pixel_9/API 35 与 API 27 各 3/3，使用真实 Service 和真实 `DashboardRuntime` 验证 Start 存储确认前定位启动次数为 0、确认后只启动一次，Activity 可见性解绑/重绑的启停幂等，以及注入式可恢复写失败后通知立即显示“等待存储恢复”、结束入口保留、定位停止并在检查点恢复后只重启一次；存储与系统 GPS 注册由测试边界隔离。
- **未验证（发布阻断）**：物理磁盘 `ENOSPC`/低存储下的通知与真实 GPS 注册停止/恢复、End/尾点与队列 actor 异常、`onTaskRemoved()` 回收竞态、尾批损失量化、最终候选版本号升级复验、热路径 Profile 重采集和真实设备长测。
...
| M3 | P0 | API 27/29 `SIGKILL` 恢复、API 27 重启数据保留与恢复首点 JVM 已验证，待尾批/异常场景/发版 | 进程异常退出与最后批次恢复 | 确认边界支持晚到订阅者读取，任务移除可等待检查点；正常批次延迟约 1 秒，异常写失败时只保证最多 16 个未确认点且不保证时间上限；设备级恢复首点和尾批损失仍未量化；重启后需用户打开应用 | 杀进程、断电或 `force-stop` 零丢点，或宣称开机自动继续 |
...
| M6 | P1；阻断发版 | API 27/29/35 30 分钟、本地完整关卡、Pixel_9/API 27 完整套件及真实 Service 3/3 seam 已通过；异常环境、低存储与真机待验收 | 生命周期并发与真机长测 | 不可见 Start 等待已禁止后台定位；控制器 4/4 JVM 与 Service 生命周期 3/3 设备 seam 已覆盖确认门禁、重绑幂等和注入式可恢复故障通知/定位编排，但 End/尾点、actor 异常、`onTaskRemoved()` 回收、真实低存储、尾批量化和真实设备仍未完成 | 最后定位点必不丢失或已验证真实道路和长时功耗 |
...
M1-M6 的核心实现已经通过本地关卡、Pixel_9 / API 35，以及 API 27/29/31/33 聚焦验证；最新 16 点背压加固后又通过完整本地构建，并在 Pixel_9 / API 35 与 API 27 各完成 23 项当前 instrumentation（`gps-core` 14/14 + 手机版 9/9）。M5 跨 API 完整权限矩阵已闭环，API 27/API 29 的公开正式旧包覆盖升级也已完成。API 27/29/35 的 30 分钟后台回归和 API 27 整机重启边界已经通过，但物理低存储、Service 其余异常环境、最终候选身份、真实设备和热路径 Profile 重采集仍未完成，因此还不能直接发版。发布候选至少还要完成以下四组关卡：
...
2. **低存储与事件顺序**：Room 实际连接的永久只读写失败已经验证存储层原子性（API 35/API 27 存储类各 13/13），新增 Runtime/Room 专项又在两档设备各 1/1、完整 `gps-core` 各 14/14 验证前 16 点保留、第 17 点拒绝和恢复后检查点确认；真实 Service 3/3 seam 又验证了 Start 确认门禁、可见性重绑，以及注入式可恢复写失败后的通知、结束入口、定位停止与检查点恢复重启。仍需制造物理 `ENOSPC`/低存储并接入真实 Room 与系统 GPS 注册，量化恢复耗时和进程终止前后的尾批损失，并补冷启动恢复首点、活动行程 Activity 重建、End/尾点、队列关闭/actor 异常和 `onTaskRemoved()` 并发。API 27 整机重启已证明当前边界是“用户打开应用后恢复”，不应写成开机自动继续。
...
Room schema 与显式迁移实现已经完成，API 27/29/33 上的 12 个 fixture/事务/损坏门禁 instrumentation 均已通过；API 27/API 29 的公开正式旧包升级也已保留真实数据库证据。新增的 Room 只读连接故障用例在 API 35/API 27 存储类各 13/13 通过，新增 Runtime/Room 背压专项各 1/1、完整 `gps-core` 各 14/14，证明连接级永久写失败不会清除已确认活动数据，并能在运行时保留 16 点尾批后恢复检查点，但不等价于物理 `ENOSPC`。API 27/29/35 的 30 分钟结果和 API 27 重启数据保留结果已完成，JVM seam 又补齐了点写失败统一失败流、确认边界 replay、任务移除等待、Start 清理竞态和 16 点有界背压；真实 Service seam 已覆盖注入故障的通知与定位编排。剩余发布风险集中为物理低存储下的真实 Room/系统 GPS 恢复、真实 Service 其余异常竞态、尾批损失量化、最终版本号候选是否仍保持同一升级结果、Profile 重采集，以及真实设备 2 小时长测。当前开机自动恢复不是已交付能力；若产品要改变这一边界，必须单独评审后台位置权限和系统启动规则。AGP 9 仍需分阶段推进，不能与异常恢复和最终候选复验塞入同一次架构改动。真实设备测试必须取得单独设备授权；当前模拟器授权不能自动扩展到 Redmi。

### 当前剩余高风险摘要

| 优先级 | 剩余项 | 已有证据 | 下一步与发布影响 |
| --- | --- | --- | --- |
| P0 | 物理低存储与尾批损失量化 | Room/SQLite 真实只读连接失败已在 API 35/API 27 存储类各 13/13 通过；Runtime/Room 专项在两档设备各 1/1、完整 `gps-core` 各 14/14 验证未确认点最多 16 个、第 17 个拒绝、恢复后原尾批确认和配额释放；Service 3/3 seam 已验证注入故障后的通知、结束入口和定位停止/重启编排；JVM seam 已验证拒绝点不会成为恢复首点的距离/速度基准 | 制造物理 `ENOSPC`/低存储并接入真实 Room 与系统 GPS，断言恢复耗时和实际注册变化；量化进程终止前后的确认点与丢点窗口，完成前不能宣称零丢点 |
| P0 | 最终候选身份与覆盖升级 | API 27/29 已用公开 `v0.2.0` 同证书验证 Room v4 无损迁移 | 提升版本号，生成最终签名 Release，再重复 API 27/API 29 覆盖升级；完成前不能发新版本 |
| P1 | Service/队列异常竞态 | JVM seam 已覆盖 Start 状态清理、检查点等待、重绑 replay 和背压停止策略；真实 Service seam 在 API 35/API 27 各 3/3 覆盖 Start 确认门禁、可见性重绑和注入式故障恢复；三档常规 30 分钟已通过 | 补活动行程 Activity 重建、End 与并发尾点、队列关闭/actor 异常、物理低存储恢复和 `onTaskRemoved()` 回收设备集成测试；当前仍是 M6 发布阻断 |
...
验证证据：M1 初始门禁为 2026-08-07 本地 `gps-core` 32/32、Pixel_9 SQLite 4/4 和 Compose 1/1；最新背压、普通存储失败输入阻断、恢复首点分段和队列取消语义加固后完整 JVM 为 `gps-core` 54/54、手机版 31/31，并通过完整本地构建。Pixel_9/API 35 与 API 27 当前完整 `gps-core` instrumentation 各 14/14、手机版各 9/9，Runtime/Room 背压专项各 1/1；存储类另为 13/13。`DashboardRuntimePersistenceTest` 现分别验证背压拒绝点不进累计器，以及普通存储错误未被确认前不再接收新定位点；两条路径恢复首点都断开旧距离/速度基线。`TripSessionEventQueueTest` 新增恢复阻塞期间取消等待后跳过尚未消费 Start 的回归。`TripRecordingServiceLifecycleInstrumentedTest` 在两档设备各 3/3 验证真实 Service 的 Start 存储确认门禁、前台服务升起、可见性重绑，以及注入式可恢复写失败后的通知、结束入口、定位停止和检查点恢复重启；该 seam 使用真实 `DashboardRuntime`，但存储与系统 GPS 仍由测试边界隔离，不覆盖物理低存储、actor 异常或 `onTaskRemoved()` 回收竞态。`RoomRuntimeBackpressureInstrumentedTest` 又在 Room 实际连接上验证前 16 点保留、第 17 点拒绝、活动行程不清除和恢复后检查点确认；摘要为 `artifacts/cargps-mobile-runtime-backpressure-summary.md`。`SqliteTripStorageInstrumentedTest.roomStorageKeepsActiveTripWhenSqliteConnectionIsReadOnly` 的存储层原子失败摘要仍见 `artifacts/cargps-mobile-storage-readonly-summary.md`。截图与 UI 树仍为初始 M1 的 `artifacts/cargps-mobile-m1-pixel9.png`、`artifacts/cargps-mobile-m1-pixel9.xml`；当前没有物理低存储或真实系统 GPS 恢复证据。
...
- `TripRecordingService.onTaskRemoved()` 在 Service 协程中等待 `DashboardRuntime.checkpointTripWritesAndAwait()`；该回调不能阻塞主线程，也不保证系统回收前完成。系统未调用回调或协程尚未完成时，以最近一次定时批次确认边界恢复；正常批次延迟约 1 秒，异常写失败时只保证未确认点最多 16 个，不保证时间上限。
...
剩余门槛：已知点序列在进程终止前后的损失窗口量化、物理低存储下 16 点边界与恢复耗时、Activity 重建/Service 重连和 `onTaskRemoved()` 并发仍未完成；Room 只读连接测试已覆盖存储层失败保留，但尚未在真实 Service 背压路径和回调竞态中验收恢复首点分段。`force-stop` 会阻止系统自动启动，整机重启当前也不会自动启动，不能与普通进程回收混为一谈。若未来要支持开机自动恢复，必须单独处理 `BOOT_COMPLETED`、后台位置权限和 Android 14/15 规则。
...
- 队列关闭会取消当前和缓冲区中的等待型命令；actor 未预期异常会关闭入口、失败等待者并拒绝后续事件，不能继续“假接收”。调用方主动取消等待时，对应完成信号会立即取消；若命令尚未开始消费，actor 会跳过它，避免生命周期结束后再产生迟到行程副作用。
- `onTaskRemoved()` 通过 Service 协程等待检查点，但仍是尽力执行，不能承诺进程回收前完成；队列只保证单进程内顺序，不提供跨进程 exactly-once 语义。
...
- 2026-08-08 最新背压、普通持久化失败输入阻断与恢复首点分段加固后 `gps-core` 54/54、手机版 31/31 JVM 通过；AndroidTest 编译、`lintDebug`、`lintVitalRelease`、Debug/Release、R8、资源压缩和 `baselineprofile:assembleBenchmarkRelease` 完整通过。随后在 `ro.boot.qemu.avd_name = Pixel_9` 的 `emulator-5554 / API 35`，以及 Gradle 识别为 `CASKA_1024x600(AVD) - 8.1.0` 的 `emulator-5556 / API 27` 上各复验完整 `gps-core` 14/14、手机版 9/9 instrumentation；Runtime/Room 背压专项各 1/1，新增 Service 生命周期 seam 各 3/3。serial 不能视为永久映射。
- `TripSessionEventQueueTest` 6/6，覆盖 Restore/FIFO、等待型 Start、关闭取消、actor 异常终止、连续 Toggle，以及恢复阻塞期间取消等待后跳过尚未消费的 Start；协调器测试确认结束前尾点进入统计、End 后点明确拒绝，Runtime 测试确认等待型 Start 返回前已发布记录状态，并验证普通存储失败与背压恢复首点都不会跨故障窗口补算距离、速度或确认点序列。
...
- `TripRecordingServiceLifecycleInstrumentedTest` 在 Pixel_9/API 35 与 API 27 各 3/3，使用真实 Service、真实 `DashboardRuntime` 和测试存储/定位边界，验证 Start 存储确认前定位启动次数保持 0、确认后只启动 1 次，Activity 可见性解绑/重绑不会产生重复定位注册，以及可恢复写失败后通知立即切换、定位停止、结束入口保留并在检查点恢复后只重启一次。
- `LocationEnginePolicyTest` 7/7；除不可见 Start 等待外，覆盖普通持久化失败与背压时停止定位，同时保留可见空闲/等待预览。
- Pixel_9 / API 35：既有运行时短路径和本轮 seam 加固后的 instrumentation 均为完整 `gps-core` 14/14、手机版 9/9，Runtime/Room 背压专项 1/1，Service 生命周期 seam 3/3；开始后为 `location` 前台服务，应用 UID 连续两次 ensure 后定位线程仍为 1，结束后历史增加一段，Home 后 Service 清除，crash buffer 无 CarGPS 记录。随后完整 30 分钟路径也通过：41 个样本、1831 秒内同一 PID、前台服务、通知和单定位线程持续；锁屏睡眠不丢失运行状态，结束后资源与 GPS 注册清理。Android 10 / API 29 与 Android 8.1 / API 27 的 41 个样本、1816 秒同级路径也通过，活动 GPS 注册始终为 1，结束后为 0；API 27 的定位线程由 `/proc` 侧车确认始终为 1。
...
- 现有 Service seam 已覆盖 Start 存储确认、空闲预览的可见性解绑/重绑，以及注入式可恢复写失败后的通知和定位恢复；Room/Runtime 专项已覆盖连接级事务失败到 Runtime 背压。仍缺少活动行程 Activity 重建、队列关闭或 actor 异常、End 与并发尾点、物理低存储与真实系统 GPS 背压/恢复、`onTaskRemoved()` 请求尚未完成即回收的设备集成断言。
...
- 冷启动恢复与首点、活动行程 Activity 重建、Service 异常重连和 `onTaskRemoved()` 并发仍需在三档 API 的异常场景中复核；30 分钟常规后台长测已通过，但没有注入这些异常竞态。存储层只读失败已通过 API 35/API 27 各 13/13，Runtime/Room 背压专项各 1/1，现有 Service seam 覆盖确认门禁、空闲预览重绑和单次可恢复写失败编排，但不能替代物理低存储或上述其余异常路径证据。
